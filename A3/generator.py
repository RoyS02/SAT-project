...
def solve_nonconsecutive(grid):
    """
    Check if sudoku respects row, column, box and orthogonal constraints
    """
    for r in range(9):
        for c in range(9):
            if grid[r][c] == 0:
...
def generate_full_nonconsecutive_solution():
    """
    Generate full solution. This is used to find a full solution for sudoku A to make
    sure the overlap for sudoku A and B will be the same. If not, both sudoku's would have 
    different solutions for the overlapping area. 
    """
...
def puzzle_from_solution(solution, filled_percentage):
    """
    Make puzzle from solution by removing percentage of values and replacing with 0.
    """
...
def varnumber(row: int, column: int, value: int, N: int = N_DEFAULT) -> int:
    """
    Convert row, column value to a unique integer for clauses
    """
...
def get_rcv(unit: int, N: int = N_DEFAULT) -> Optional[Tuple[int, int, int]]:
    """
    Get real values back out of varnumber encoding. Inverse of varnumber.
    """
    if unit <= 0:
        return None
...
def apply_truths_to_grid(grid: List[List[int]], varnums: List[int]) -> None:
    """
    Put values in sudoku grid from RCV encoding.
    """
    for vn in varnums:
        rcv = get_rcv(vn)
        if rcv is None:
...
def extract_overlapping_cells(grid_A: List[List[int]], k: int) -> List[List[int]]:
    """
...
def generate_twodoku_puzzles_from_scratch(
    ...
    """
    Steps for generating TwoDoku's consisting of sudoku A and B
    1. Generate sudoku A as solved, respecting all set rules
    2. Snapshot the overlapping area of sudoku A
    3. Take snapshot and place in the corresponding area of sudoku B
    4. Generate solution for sudoku B with the overlapping area fixed based on sudoku A
       - If no SAT solution can be found, restart with new solution for sudoku A
    5. Generate puzzles from both solution by removing percentage of grid and replacing with 0's
    6. Make sure that puzzles have identical overlapping areas
    """
    ...
    while True:
        ...
        # Step 2
        groups_A = extract_overlapping_cells(solution_A, k)
        ...
        # Make sure 0 and clues are identical between A and B
